        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div class="lg:col-span-7">
                <div class="crm-card p-0 overflow-hidden">
                    ...
                    {{-- Desktop & Tablet Table (No horizontal scrollbar needed) --}}
                    <div class="hidden sm:block">
                        <table class="w-full text-sm text-left border-collapse">
                            <thead>
                                <tr class="bg-gray-50/80 border-b border-gray-100">
                                    <th class="px-3.5 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-gray-400">Pengiriman</th>
                                    ...
                                    <th class="px-3.5 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-gray-400">Status</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr class="hover:bg-gray-50/80 transition-colors">
                                        <td class="px-3.5 py-2.5 whitespace-nowrap">
                                            <a href="{{ route('admin.shipments.show', $shipment) }}"
                                               class="text-xs font-bold text-gray-900 hover:text-primary transition-colors block">
                                                {{ $shipment->shipment_number }}
                                            </a>
                                        </td>
                                        <td class="px-3.5 py-2.5">
                                            <span class="text-xs text-gray-700 font-medium block truncate max-w-[120px] xl:max-w-[160px]"
                                                  title="{{ $shipment->customer->company_name ?? '-' }}">
                                                {{ $shipment->customer->company_name ?? '-' }}
                                            </span>
                                        </td>
                                        <td class="px-3.5 py-2.5">
                                            <span class="text-xs text-gray-500 block truncate max-w-[130px] xl:max-w-[180px]"
                        ...
                        @forelse ($recentShipments as $shipment)
                            <a href="{{ route('admin.shipments.show', $shipment) }}" class="p-3 block hover:bg-gray-50 transition-colors">
                                <div class="flex items-center justify-between gap-2 mb-1">
                                    <span class="text-xs font-bold text-gray-900">{{ $shipment->shipment_number }}</span>
                                    <x-badge :status="$shipment->status" />
                                </div>
                                <div class="flex items-center justify-between text-xs text-gray-500">
                                    <span class="truncate max-w-[140px] font-medium text-gray-700">{{ $shipment->customer->company_name ?? '-' }}</span>
            ...
            <div class="lg:col-span-5">
                <div class="crm-card h-full">
                    ...
                    <div class="flex items-center justify-between pb-3 mb-3 border-b border-gray-100">
                        ...
                        <span class="flex items-center gap-1.5 text-[10px] text-emerald-600 font-semibold">
                                ...
                                @foreach ($trackingUpdates as $update)
                                    <div class="relative">
                                        ...
                                        <div class="flex items-start justify-between gap-2">
                                            <div>
                                                <p class="text-xs font-bold text-gray-900">{{ $update->location }}</p>
                                                <p class="text-[11px] text-gray-500 mt-0.5">
                                                    {{ $update->shipment->shipment_number ?? '-' }}
                                                </p>
                                                @if ($update->description)
                                                    <p class="text-[10px] text-gray-500 mt-0.5 bg-gray-50 p-1.5 rounded-btn border border-gray-100">{{ $update->description }}</p>
                                                @endif
                                            </div>
                            ...
                            @foreach ($activeShipments as $shipment)
                                <div class="p-3 rounded-btn bg-gray-50 border border-gray-100 space-y-2 hover:border-blue-100 transition-colors">
                                    ...
                                    <p class="text-[11px] text-gray-500 truncate">
                                        ...
                                        &middot; {{ $shipment->origin }} &rarr; {{ $shipment->destination }}
                                    </p>
                                    <div class="flex items-center gap-2 pt-0.5">
                                        <span class="text-[10px] font-semibold text-info truncate max-w-[80px]">{{ $shipment->origin }}</span>
                                        <div class="flex-1 h-1 bg-blue-100 rounded-full overflow-hidden">
                                            <div class="h-full bg-info rounded-full" style="width: 60%"></div>
                                        </div>
                                        <span class="text-[10px] text-gray-400 truncate max-w-[80px]">{{ $shipment->destination }}</span>
                                    </div>
                                </div>
                            @endforeach
                        @else
        ...
        <div class="crm-card space-y-4" x-data="{ activeIndex: null, activeLabel: '', activeCount: 0, activeX: 0, activeY: 0 }">
            ...
            <div class="relative pt-1">
                <div class="flex items-stretch gap-3">
                    {{-- Y-Axis Scale Labels --}}
                    <div class="flex flex-col justify-between text-[10px] font-semibold text-gray-400 text-right pr-1 select-none" style="height: 130px;">
                    ...
                    <div class="flex-1 relative">
                        <svg viewBox="0 0 640 130" class="w-full h-32 overflow-visible" preserveAspectRatio="none">
                            <defs>
                        ...
                        <div x-show="activeIndex !== null" x-cloak
                             class="absolute pointer-events-none bg-gray-900/90 text-white text-[11px] rounded-btn py-1.5 px-3 shadow-lg transform -translate-x-1/2 -translate-y-full transition-all duration-150 backdrop-blur-xs"
