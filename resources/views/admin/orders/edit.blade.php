<x-app-layout>
    <x-slot name="header">
        <h2 class="font-poppins font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Ubah Pesanan') }} — {{ $order->order_number }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-soft sm:rounded-card border border-gray-100 p-6">
                ...
                <form method="POST" action="{{ route('admin.orders.update', $order) }}" 
                      x-data="{
                          items: {{ Js::from($order->items->map(fn($i) => ['id' => $i->id, 'product_id' => $i->product_id, 'quantity' => $i->quantity, 'unit' => $i->unit, 'notes' => $i->notes ?? ''])) }},
                          products: {{ Js::from($products->map(fn($p) => ['id' => $p->id, 'name' => $p->name, 'unit' => $p->unit])) }},
                          addItem() {
                              this.items.push({ id: null, product_id: '', quantity: 1, unit: 'Pcs', notes: '' });
                          },
                          removeItem(index) {
                    ...
                    <div class="space-y-4 mb-6">
                        <h3 class="font-poppins font-semibold text-lg text-gray-800 border-b pb-2">Informasi Pesanan</h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label for="order_date" class="block text-sm font-medium text-gray-700">Tanggal Pesanan <span class="text-primary">*</span></label>
                        ...
                        <div>
                            ...
                            <select id="status" name="status" required
                                ...
                                <option value="PENDING" @selected(old('status', $order->status->value) == 'PENDING')>Menunggu</option>
                                <option value="PROCESSING" @selected(old('status', $order->status->value) == 'PROCESSING')>Diproses</option>
                                <option value="COMPLETED" @selected(old('status', $order->status->value) == 'COMPLETED')>Selesai</option>
                                <option value="CANCELLED" @selected(old('status', $order->status->value) == 'CANCELLED')>Dibatalkan</option>
                            </select>
                        </div>

                        <div>
                            <label for="notes" class="block text-sm font-medium text-gray-700">Catatan Pesanan</label>
                            <textarea id="notes" name="notes" rows="2" placeholder="Catatan tambahan untuk pesanan ini..."
                    ...
                    <div class="space-y-4 mb-6">
                        <div class="flex justify-between items-center border-b pb-2">
                            <h3 class="font-poppins font-semibold text-lg text-gray-800">Daftar Item Pesanan</h3>
                            ...
                            <div class="bg-gray-50 p-4 rounded-card space-y-3">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
                                    <div class="md:col-span-4">
                                        <label class="block text-xs font-medium text-gray-600">Produk <span class="text-primary">*</span></label>
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        <input type="number" step="0.01" min="0.01" :name="'items[' + index + '][quantity]'" x-model="item.quantity" placeholder="Contoh: 100" required
                                               class="mt-1 block w-full rounded-btn border-gray-300 focus:border-primary focus:ring-primary shadow-sm text-sm">
                                    </div>

                                    <div class="md:col-span-2">
                                        <label class="block text-xs font-medium text-gray-600">Unit <span class="text-primary">*</span></label>
                                        <input type="text" :name="'items[' + index + '][unit]'" x-model="item.unit" placeholder="Sak, Dus, Box, Pcs..." required
                                               class="mt-1 block w-full rounded-btn border-gray-300 focus:border-primary focus:ring-primary shadow-sm text-sm">
                                    </div>

                                    <div class="md:col-span-4">
                                        <label class="block text-xs font-medium text-gray-600">Keterangan</label>
                                        <input type="text" :name="'items[' + index + '][notes]'" x-model="item.notes" placeholder="Contoh: Semen Portland"
                                               class="mt-1 block w-full rounded-btn border-gray-300 focus:border-primary focus:ring-primary shadow-sm text-sm">
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="flex items-center justify-end gap-4">
                        ...
                        <button type="submit" class="px-6 py-2 bg-primary text-white rounded-btn font-semibold hover:bg-red-700 transition">Perbarui Pesanan</button>
                    </div>
                </form>
