<x-app-layout>
    <x-slot name="header"><h2 class="font-poppins font-semibold text-xl text-gray-800 leading-tight">{{ __('Add New Vehicle') }}</h2></x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-soft sm:rounded-card border border-gray-100 p-6">
                <form method="POST" action="{{ route('admin.vehicles.store') }}">
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input id="plate_number" type="text" name="plate_number" value="{{ old('plate_number') }}" placeholder="BK 1234 XX" required class="mt-1 block w-full rounded-btn border-gray-300 focus:border-primary focus:ring-primary shadow-sm @error('plate_number') border-primary @enderror">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input id="brand" type="text" name="brand" value="{{ old('brand') }}" placeholder="Hino, Isuzu, dll" class="mt-1 block w-full rounded-btn border-gray-300 focus:border-primary focus:ring-primary shadow-sm">
                            </div>
                            <div>
                                <label for="capacity" class="block text-sm font-medium text-gray-700">Kapasitas (Kg) <span class="text-primary">*</span></label>
                                <input id="capacity" type="number" step="0.01" min="0" name="capacity" value="{{ old('capacity') }}" required class="mt-1 block w-full rounded-btn border-gray-300 focus:border-primary focus:ring-primary shadow-sm @error('capacity') border-primary @enderror">
                        ...
                        <div>
                            ...
                            <textarea id="notes" name="notes" rows="2" class="mt-1 block w-full rounded-btn border-gray-300 focus:border-primary focus:ring-primary shadow-sm">{{ old('notes') }}</textarea>
                        </div>
                    </div>
                    <div class="mt-6 flex items-center justify-end gap-4">
                        ...
                        <button type="submit" class="px-6 py-2 bg-primary text-white rounded-btn font-semibold hover:bg-red-700 transition">Save Vehicle</button>
                    </div>
                </form>
            </div>
        </div>
